...
examples of usage
-----------------
iniparser.py config.ini                                 // returns list of sections
iniparser.py config.ini {SECTION                        // returns list of items in SECTION
iniparser.py config.ini {SECTION} {OPTION}              // returns value of OPTION in SECTION
iniparser.py config.ini {SECTION} {OPTION} {VALUE}      // sets VALUE of OPTION in SECTION

iniparser.py -d config.ini {SECTION}                    // deletes SECTION
iniparser.py -d config.ini {SECTION} {OPTION}           // deletes OPTION in SECTION
iniparser.py -d config.ini {SECTION} {OPTION} {VALUE}   // deletes value of OPTION in SECTION when VALUE matches
...
def delete(inifile,section=None,option=None,value=None):
	...
	if section is None and option is None and value is None:
		...
		if ini.get(section,option) != value:
			raise Exception("value of option '%s' in section '%s' (value: '%s') mismatches required '%s'" % (option,section,ini.get(section,option),value))
...
def error(message):
	"""print message nicely to strerr"""
	...
	try:
		...
		if isinstance(e, UsageError):
			try:
				...
				print 'Error: Can not display usage.'
		sys.exit(2)
